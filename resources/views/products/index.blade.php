        ...
        <div class="row mb-4">
            <div class="col-md-8 offset-md-2 text-center">
                <form action="{{ route('products.index') }}" method="GET" class="form-inline justify-content-center">
                    <input type="text" name="query" class="form-control mr-2" placeholder="Buscar productos..." value="{{ request('query') }}" style="width: 100%; max-width: 400px;">
                ...
                <div class="col-md-4 mb-4">
                    <div class="card h-100">
                        @if($product->image_url)
                            <img src="{{ $product->image_url }}" class="card-img-top" alt="{{ $product->name }}" style="width: 150px; height: 150px; object-fit: cover; margin: auto;">
                        @else
                            <img src="https://via.placeholder.com/150" class="card-img-top" alt="Imagen no disponible" style="width: 150px; height: 150px; object-fit: cover; margin: auto;">
                        @endif
                        <div class="card-body d-flex flex-column">
                            ...
                            <div class="mt-auto">
                                <a href="{{ route('products.show', $product->id) }}" class="btn btn-info btn-block">Ver Detalles</a>
                                <a href="{{ route('products.edit', $product->id) }}" class="btn btn-warning btn-block">Editar</a>
                                <form action="{{ route('products.destroy', $product->id) }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-danger btn-block" onclick="return confirm('¿Estás seguro de que deseas eliminar este producto?')">Eliminar</button>
                                </form>
